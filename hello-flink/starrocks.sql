...
load label load_emp_37(
     ...
     format as "ORC"
     (emp_no,emp_name,dept_no,dept_name,sex,create_dt,salary)
     --  我们可以从文件的目录中提取出变量，比如我设置了变量dt，sr匹配到了文件hdfs://namenode:9000/user/hive/warehouse/mydb.db/emp_info/dt=2024-06-27/000000_0，那么会在该目录下找包含【dt=xxxx】的地方，找到的就是dt=2024-06-27，因此变量dt的值就是2024-06-27
...
SELECT * FROM information_schema.loads where LABEL='load_emp_37';
...
select tracking_log from information_schema.load_tracking_logs where job_id=17025