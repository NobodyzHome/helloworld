...
show partitions from mydb.mv_non_partition;

# 本章内容：创建一个无分区的异步物化视图
# 1.创建视图时，没有partition by语句，则代表创建一个无分区的视图
...
group by t2.name;
# 2.查询视图的任务名称
select task_name from information_schema.materialized_views where table_name='mv_non_partition';
# 3.查看视图的任务状态
# +---------+-------------------+-------------------------------------------------------+--------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------+
# |TASK_NAME|CREATE_TIME        |SCHEDULE                                               |DATABASE|DEFINITION                                                                                                                                                                                                                                                             |EXPIRE_TIME|
# +---------+-------------------+-------------------------------------------------------+--------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------+
# |mv-15191 |2024-08-08 12:43:29|PERIODICAL (START 2024-08-08T12:43:40 EVERY(1 MINUTES))|mydb    |insert overwrite `mv_non_partition` SELECT `mydb`.`t2`.`name` AS `sex_name`, count(`mydb`.`t1`.`id`) AS `cnt`, now() AS `ts`                                                                                                                                           |null       |
# |         |                   |                                                       |        |FROM `mydb`.`base_table1` AS `t1` , `mydb`.`dim_table_sex` AS `t2`                                                                                                                                                                                                     |           |
# |         |                   |                                                       |        |WHERE `mydb`.`t1`.`sex` = `mydb`.`t2`.`id`                                                                                                                                                                                                                             |           |
# |         |                   |                                                       |        |GROUP BY `mydb`.`t2`.`name`                                                                                                                                                                                                                                            |           |
# +---------+-------------------+-------------------------------------------------------+--------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------+
select * from information_schema.tasks where task_name='mv-15191';
# 4.查看任务实例的执行情况，可以看到8月9日的任务都已经正常完成了
# +------------------------------------+-------------------+-------+--------+
# |query_id                            |finish_time        |state  |progress|
# +------------------------------------+-------------------+-------+--------+
# |4a306acf-561a-11ef-a9a2-0242ac120003|2024-08-09 06:40:39|SUCCESS|100%    |
# |2673b461-561a-11ef-a9a2-0242ac120003|2024-08-09 06:39:39|SUCCESS|100%    |
# |02b6fdf2-561a-11ef-a9a2-0242ac120003|2024-08-09 06:38:39|SUCCESS|100%    |
# +------------------------------------+-------------------+-------+--------+
select query_id,finish_time,state,progress from information_schema.task_runs where task_name='mv-15191' limit 3;
# 5.查看视图的数据，发现数据的ts还是8月8日。这是因为基表中数据没有变化，当任务触发后，发现基表数据没有被改变，就不需要刷新视图了，所以视图中数据的ts没有改变。
# +--------+---+---------------------+
# |sex_name|cnt|ts                   |
# +--------+---+---------------------+
# |male    |2  |2024-08-08 12:46:39.0|
# |male1   |1  |2024-08-08 12:46:39.0|
# |male2   |1  |2024-08-08 12:46:39.0|
# |男      |4  |2024-08-08 12:46:39.0|
# +--------+---+---------------------+
select * from mydb.mv_non_partition;
# 6.向基表中插入数据，改变基表中的数据
insert into mydb.base_table1 values('2024-07-15',19,'wang dong',27,1);
# 7.下一次视图任务执行完成后，再查询视图中的数据，发现ts字段已经被改变了
# +--------+---+---------------------+
# |sex_name|cnt|ts                   |
# +--------+---+---------------------+
# |男       |5  |2024-08-09 06:42:39.0|
# |male    |1  |2024-08-09 06:42:39.0|
# |male1   |1  |2024-08-09 06:42:39.0|
# |male2   |1  |2024-08-09 06:42:39.0|
# +--------+---+---------------------+
select * from mydb.mv_non_partition;
# 8.至于异步物化视图怎么知道基表的数据有没有改变，大概率应该是用show partitions的结果，每次向一个分区load完数据，VisibleVersion会+1，VisibleVersionTime会改变，用这两个字段就能判断出数据是否有变动。
# +-----------+-------------+--------------+-------------------+------------------+------+------------+--------------------------------------------------------------------------+---------------+-------+--------------+-------------+-------------------+------------------------+--------+----------+--------+
# |PartitionId|PartitionName|VisibleVersion|VisibleVersionTime |VisibleVersionHash|State |PartitionKey|Range                                                                     |DistributionKey|Buckets|ReplicationNum|StorageMedium|CooldownTime       |LastConsistencyCheckTime|DataSize|IsInMemory|RowCount|
# +-----------+-------------+--------------+-------------------+------------------+------+------------+--------------------------------------------------------------------------+---------------+-------+--------------+-------------+-------------------+------------------------+--------+----------+--------+
# |15058      |p20240715    |13            |2024-08-09 06:42:00|0                 |NORMAL|dt          |[types: [DATE]; keys: [2024-07-15]; ..types: [DATE]; keys: [2024-07-16]; )|id             |2      |1             |HDD          |9999-12-31 15:59:59|null                    |973B    |false     |1       |
# |14768      |p20240730    |5             |2024-08-02 10:14:02|0                 |NORMAL|dt          |[types: [DATE]; keys: [2024-07-30]; ..types: [DATE]; keys: [2024-07-31]; )|id             |2      |1             |HDD          |9999-12-31 15:59:59|null                    |1007B   |false     |4       |
# |14867      |p20240731    |2             |2024-08-02 10:13:24|0                 |NORMAL|dt          |[types: [DATE]; keys: [2024-07-31]; ..types: [DATE]; keys: [2024-08-01]; )|id             |2      |1             |HDD          |9999-12-31 15:59:59|null                    |968B    |false     |1       |
# |14738      |p20240801    |2             |2024-08-02 07:42:54|0                 |NORMAL|dt          |[types: [DATE]; keys: [2024-08-01]; ..types: [DATE]; keys: [2024-08-02]; )|id             |2      |1             |HDD          |9999-12-31 15:59:59|null                    |962B    |false     |1       |
# |14705      |p20240802    |3             |2024-08-02 08:09:42|0                 |NORMAL|dt          |[types: [DATE]; keys: [2024-08-02]; ..types: [DATE]; keys: [2024-08-03]; )|id             |2      |1             |HDD          |9999-12-31 15:59:59|null                    |966B    |false     |1       |
# +-----------+-------------+--------------+-------------------+------------------+------+------------+--------------------------------------------------------------------------+---------------+-------+--------------+-------------+-------------------+------------------------+--------+----------+--------+
...
select * from (
                  select
                      ...
                      count(t1.id) cnt,
                      now() ts
                  from mydb.base_table1 t1
                           join mydb.dim_table_sex t2
                                on
                                    t1.sex=t2.id
                  group by t2.name
              ) t
